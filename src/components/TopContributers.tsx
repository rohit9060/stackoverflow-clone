...
import { convertDateToRelativeTime } from "@/lib";
import { avatars } from "@/models/client/config";

const Notification = ({ user }: { user: Models.User<IUserPrefs> }) => {
  return (
    <figure
      className={cn(
...
export async function TopContributers() {
  const topUsers = await users.list<IUserPrefs>([Query.limit(10)]);
