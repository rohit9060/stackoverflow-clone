...
const Page = async ({
  ...
  const [question, answers, upvotes, downvotes, comments] = await Promise.all([
    database.getDocument(dbName, questionCollection, params.quesId),
    ...
    database.listDocuments(dbName, commentCollection, [
      Query.equal("type", "question"),
      Query.equal("typeId", params.quesId),
      Query.orderDesc("$createdAt"),
    ]),
  ...
  const author = await users.get<IUserPrefs>(question.authorId);
  [comments.documents, answers.documents] = await Promise.all([
    Promise.all(
      comments.documents.map(async (comment) => {
        const author = await users.get<IUserPrefs>(comment.authorId);
        return {
          ...comment,
          author: {
            $id: author.$id,
    ...
    Promise.all(
      answers.documents.map(async (answer) => {
        const [author, comments, upvotes, downvotes] = await Promise.all([
          users.get<IUserPrefs>(answer.authorId),
          database.listDocuments(dbName, commentCollection, [
